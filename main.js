...
// Initially start with 4 enemies
let wave_size = 4;
spawn_enemy(4);
...
function update() {
	// Wave logic
	if (!enemy_store.length) {
		wave_size += 2;
		spawn_enemy(wave_size);
	}
	...
	if (shot) {
		...
		if (document.body.contains(bullet)) {
			...
			// if enemy(ies) hit: remove bullet
			if (hit) {
				shot = false;
				bullet.parentNode.removeChild(bullet);
			}
